...
func TestListModel_Init(t *testing.T) {
	...
	fakeAppState := application.State{Selected: 1}
	...
	lm = New(
		context.TODO(),
		storage,
		&application.State{Group: "Group 2"},
		&mocklogger.Logger{},
	)
	lm.Init()
	...
	lm = New(
		context.TODO(),
		storage,
		&application.State{}, // we don't need app state, as error should be reported before we can even use it
		&mocklogger.Logger{},
	)
	teaCmd = lm.Init()
...
func TestListModel_copyItem(t *testing.T) {
	...
	storageShouldFail := true
	storage := testutils.NewMockStorage(storageShouldFail)
	lm := New(context.TODO(), storage, &application.State{}, &mocklogger.Logger{})
	teaCmd := lm.copyItem()
...
func TestUpdate_TeaSizeMsg(t *testing.T) {
	...
	require.Positive(t, model.Height())
	require.Positive(t, model.Width())
}
